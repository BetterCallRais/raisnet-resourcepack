...
public final class LegendaryRais extends JavaPlugin implements Listener, CommandExecutor {
    ...
    public void onEnable() {
        ...
        getLogger().info("LegendaryRais v3.7.2 HAND-ARMOR-VISOR FIX enabled. Bedrock hand binding + visible native armor attachables + forced visor equipment refresh active.");
    ...
    private void applyV300ConfigMigration() {
        ...
        getConfig().set("config-version", 372);
        ...
        saveConfig();
        getLogger().info("Migrated LegendaryRais config to v3.7.2 Native-Stable Self-Deploy defaults.");
    }
    ...
    private void deployBundledGeyserAssets() {
        try {
            ...
            getLogger().info("[BEDROCK-AUTOSETUP] v3.7.2 pack + dual mapping terpasang otomatis di "+geyserDir.getPath());
        } catch (Throwable ex) {
    ...
    ItemStack createSoulTideKatana(boolean menuCopy) {
        ...
        pdc.set(waterAssetVersionKey, PersistentDataType.INTEGER, 372);
    ...
    private boolean matchesLegendaryWeapon(ItemStack item, Material material, String weaponId, int modelData,
        ...
        if (itemModelKey.equals(currentModel) || oldSoulModel || modelData == readCustomModelNumber(meta)) {
            ...
            meta.getPersistentDataContainer().set(waterAssetVersionKey, PersistentDataType.INTEGER, 372);
    ...
    private ItemStack refreshWaterItem(ItemStack old) {
        ...
        if (ver != null && ver >= 372 && expected.equals(model) && old.getType() == material && readUnbreakable(meta)) return old;
    ...
    public void onWaterAssetJoin(PlayerJoinEvent event) {
        new BukkitRunnable() {
            @Override public void run() {
                ...
                if (fixed > 0) {
                    p.sendMessage("§bLegendaryRais §8• §a" + fixed + " Water Relic item diperbarui ke asset v3.7.2.");
