...
/**
 * v3.7.2 Native-Stable Self-Deploy: 3D Java armor overlays, real MAX_HEALTH, weapon passives, unique armor skills and Bedrock parity. The two premium water relics remain in LegendaryRais core
...
public final class ArsenalExpansion implements Listener, CommandExecutor, TabCompleter {
    ...
    public void enable() {
        ...
        clearAllArmorVisuals(); // Java display overlay permanently disabled: native armor only, no follower object. Bedrock uses slot-bound attachables.
        runSelfCheck();
    }
    ...
    private void runBedrockDiagnostic(Player p){
        ...
        p.sendMessage("§b§lLegendaryRais v3.7.2 DIAGNOSTIC");
        if(geyser==null){
        ...
        try{
            ...
            if(mf!=null)for(File f:mf){
                try{
                    ...
                    if(s.contains("raisnet372:")&&s.contains("\"format_version\": 2"))currentMaps++;
            ...
            p.sendMessage((currentMaps==1?"§a✔":"§c✘")+" v3.7.2 mapping terdeteksi: §f"+currentMaps+" §7(total Legendary: "+legendaryMaps+")");
            p.sendMessage((currentPacks==1?"§a✔":"§c✘")+" v3.7.2 Bedrock pack terdeteksi: §f"+currentPacks+" §7(total Legendary: "+legendaryPacks+")");
            if(legendaryMaps>1)p.sendMessage("§e⚠ Ada beberapa LegendaryRais mapping. Hapus versi lama agar tidak collision.");
            if(legendaryPacks>1)p.sendMessage("§e⚠ Ada beberapa LegendaryRais Bedrock pack. Sisakan v3.7.2 saja.");
    ...
    @Override public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(cmd.getName().equalsIgnoreCase("risdiag")){
            if(!(sender instanceof Player p)){sender.sendMessage("LegendaryRais v3.7.2 diagnostic harus dijalankan oleh player.");return true;}
        ...
        if(cmd.getName().equalsIgnoreCase("risrepair")){
            ...
            p.sendMessage("§aLegendaryRais repair selesai. §f"+fixed+" §aitem diperbarui ke asset v3.7.2.");
            return true;
        }
    ...
    private ItemStack createArmorPiece(String set,String piece){
        ...
        m.getPersistentDataContainer().set(assetVersionKey,PersistentDataType.INTEGER,372);
        item.setItemMeta(m);
        return item;
    }
    ...
    @EventHandler public void armorHealthJoin(PlayerJoinEvent e){
        Bukkit.getScheduler().runTaskLater(plugin,()->{
            ...
            syncLegendaryHealth(p);
            applyHelmetMode(p,helmetMode(p));
            if(fixed>0)p.sendMessage("§bLegendaryRais §8• §a"+fixed+" item lama diperbarui ke asset v3.7.2.");
        },5L);
    }
    ...
    public void rightClickEquipLegendaryArmor(PlayerInteractEvent e){
        ...
        Bukkit.getScheduler().runTask(plugin,()->{
            syncLegendaryHealth(p);
            if("helmet".equals(piece))applyHelmetMode(p,helmetMode(p));
        });
    ...
    private boolean applyHelmetMode(Player p,String mode){
        ...
        int cmd=armorCmd(set,"helmet")+("open".equals(suffix)?1000:0);
        ...
        try{
            ...
            plugin.getLogger().warning("Helmet visor model update failed for "+set+": "+ex.getMessage());
        }
        meta.getPersistentDataContainer().set(helmetModeKey,PersistentDataType.STRING,suffix.toUpperCase(Locale.ROOT));
        meta.getPersistentDataContainer().set(assetVersionKey,PersistentDataType.INTEGER,372);

        ItemStack refreshed=helmet.clone();
        refreshed.setItemMeta(meta);
        p.getInventory().setHelmet(refreshed);
        p.updateInventory();
        forceHelmetEquipmentRefresh(p,refreshed);
        bar(p,"HELMET VISOR • "+suffix.toUpperCase(Locale.ROOT));
        try{p.playSound(p.getLocation(),Sound.UI_BUTTON_CLICK,.75f,"open".equals(suffix)?1.45f:.82f);}catch(Throwable ignored){}
        return true;
    }

    private void forceHelmetEquipmentRefresh(Player wearer,ItemStack helmet){
        // Do not remove the real helmet from the server. Send one visual clear packet,
        // then the refreshed helmet packet on the next tick. This forces Geyser/Bedrock
        // and Java viewers to rebuild the equipped attachable without duplication risk.
        ItemStack air=new ItemStack(Material.AIR);
        for(Player viewer:Bukkit.getOnlinePlayers()){
            try{viewer.sendEquipmentChange(wearer,EquipmentSlot.HEAD,air);}catch(Throwable ignored){}
        }
        Bukkit.getScheduler().runTaskLater(plugin,()->{
            if(!wearer.isOnline())return;
            ItemStack current=wearer.getInventory().getHelmet();
            ItemStack send=(current==null||current.getType()==Material.AIR)?helmet:current;
            for(Player viewer:Bukkit.getOnlinePlayers()){
                try{viewer.sendEquipmentChange(wearer,EquipmentSlot.HEAD,send);}catch(Throwable ignored){}
            }
    ...
    private ItemStack refreshLegendaryItem(ItemStack old){
        ...
        if(expansionWeapon(wid)){
            ...
            if(ver!=null&&ver>=372&&old.getType()==expected)return old;
    ...
    private String weaponId(ItemStack item){
        ...
        if(expansionWeapon(id)&&(ver==null||ver<372)){
    ...
    private void runSelfCheck(){
        ...
        if(plugin.getConfig().getBoolean("armor.java-3d-overlay",false))errors.add("Java follower overlay must stay disabled");
    ...
    @EventHandler(priority=EventPriority.HIGHEST,ignoreCancelled=true)
    public void legendaryMeleeDamageFloor(EntityDamageByEntityEvent e){
        ...
        double desired;
        if("soul_tide_katana_v4".equals(wid))desired=plugin.getConfig().getDouble("base-hit.sword",38.0);
        else if("abyss_leviathan_trident_v3".equals(wid))desired=plugin.getConfig().getDouble("base-hit.trident",44.0);
        else if(expansionWeapon(wid))desired=baseDamage(wid);
        else return;

        // Full Legendary armor owns its own final damage normalization.
        if(target instanceof Player tp && fullSet(tp)!=null)return;

        double beforeHealth=target.getHealth();
        double beforeAbsorb=target.getAbsorptionAmount();
        e.setDamage(Math.max(e.getDamage(),desired));
        Bukkit.getScheduler().runTask(plugin,()->{
            if(!target.isValid()||target.isDead())return;
            double before=beforeHealth+beforeAbsorb;
            double now=target.getHealth()+target.getAbsorptionAmount();
            double actual=Math.max(0.0,before-now);
            double missing=desired-actual;
            if(missing<=.01)return;
            double absorb=target.getAbsorptionAmount();
            double fromAbsorb=Math.min(absorb,missing);
            if(fromAbsorb>0){target.setAbsorptionAmount(Math.max(0.0,absorb-fromAbsorb));missing-=fromAbsorb;}
            if(missing>0&&target.isValid()&&!target.isDead())target.setHealth(Math.max(0.0,target.getHealth()-missing));
        });
    }
