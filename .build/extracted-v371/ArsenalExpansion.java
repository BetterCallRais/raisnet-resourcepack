...
/**
 * v3.7.1 Native-Stable Self-Deploy: 3D Java armor overlays, real MAX_HEALTH, weapon passives, unique armor skills and Bedrock parity. The two premium water relics remain in LegendaryRais core
...
public final class ArsenalExpansion implements Listener, CommandExecutor, TabCompleter {
    ...
    public void enable() {
        ...
        clearAllArmorVisuals(); // Java display overlay permanently disabled: native armor only, no follower object.
        runSelfCheck();
    }
    ...
    private void runBedrockDiagnostic(Player p){
        ...
        p.sendMessage("§b§lLegendaryRais v3.7.1 DIAGNOSTIC");
        if(geyser==null){
        ...
        try{
            ...
            if(mf!=null)for(File f:mf){
                try{
                    ...
                    if(s.contains("raisnet371:")&&s.contains("\"format_version\": 2"))currentMaps++;
            ...
            p.sendMessage((currentMaps==1?"§a✔":"§c✘")+" v3.7.1 mapping terdeteksi: §f"+currentMaps+" §7(total Legendary: "+legendaryMaps+")");
            p.sendMessage((currentPacks==1?"§a✔":"§c✘")+" v3.7.1 Bedrock pack terdeteksi: §f"+currentPacks+" §7(total Legendary: "+legendaryPacks+")");
            if(legendaryMaps>1)p.sendMessage("§e⚠ Ada beberapa LegendaryRais mapping. Hapus versi lama agar tidak collision.");
            if(legendaryPacks>1)p.sendMessage("§e⚠ Ada beberapa LegendaryRais Bedrock pack. Sisakan v3.7.1 saja.");
    ...
    @Override public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(cmd.getName().equalsIgnoreCase("risdiag")){
            if(!(sender instanceof Player p)){sender.sendMessage("LegendaryRais v3.7.1 diagnostic harus dijalankan oleh player.");return true;}
        ...
        if(cmd.getName().equalsIgnoreCase("risrepair")){
            ...
            p.sendMessage("§aLegendaryRais repair selesai. §f"+fixed+" §aitem diperbarui ke asset v3.7.1.");
            return true;
        }
    ...
    private ItemStack createArmorPiece(String set,String piece){
        ...
        m.getPersistentDataContainer().set(assetVersionKey,PersistentDataType.INTEGER,371);
        item.setItemMeta(m);
        return item;
    }
    ...
    @EventHandler public void armorHealthJoin(PlayerJoinEvent e){
        Bukkit.getScheduler().runTaskLater(plugin,()->{
            ...
            int fixed=refreshAllLegendaryItems(p);
            syncLegendaryHealth(p);
            if(fixed>0)p.sendMessage("§bLegendaryRais §8• §a"+fixed+" item lama diperbarui ke asset v3.7.1.");
        },5L);
    }
    ...
    private boolean applyHelmetMode(Player p,String mode){
        ...
        meta.getPersistentDataContainer().set(assetVersionKey,PersistentDataType.INTEGER,371);
    ...
    private ItemStack refreshLegendaryItem(ItemStack old){
        ...
        if(expansionWeapon(wid)){
            ...
            if(ver!=null&&ver>=371&&old.getType()==expected)return old;
    ...
    private String weaponId(ItemStack item){
        ...
        if(expansionWeapon(id)&&(ver==null||ver<371)){
