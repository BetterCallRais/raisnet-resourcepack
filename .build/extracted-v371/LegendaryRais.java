...
public final class LegendaryRais extends JavaPlugin implements Listener, CommandExecutor {
    ...
    public void onEnable() {
        ...
        getLogger().info("LegendaryRais v3.7.1 NATIVE-STABLE SELF-DEPLOY enabled. Reliable damage floors + native Java armor + dual Bedrock mappings + automatic Geyser deployment active.");
    ...
    private void applyV300ConfigMigration() {
        ...
        getConfig().set("config-version", 371);
        ...
        getConfig().set("armor.java-3d-overlay", false);
        ...
        saveConfig();
        getLogger().info("Migrated LegendaryRais config to v3.7.1 Native-Stable Self-Deploy defaults.");
    }

    private void deployBundledGeyserAssets() {
        try {
            ...
            if (children != null) {
                for (File f : children) {
                    ...
                    if (n.equals("geyser-spigot") || n.startsWith("geyser-")) { geyserDir=f; break; }
                }
            }
            if (geyserDir == null) geyserDir=new File(pluginsDir,"Geyser-Spigot");
            File packs=new File(geyserDir,"packs");
            File maps=new File(geyserDir,"custom_mappings");
            packs.mkdirs(); maps.mkdirs();

            // Remove only LegendaryRais files; never touch other Geyser packs/mappings.
            File[] oldPacks=packs.listFiles();
            if(oldPacks!=null)for(File f:oldPacks)if(f.isFile()&&f.getName().toLowerCase(Locale.ROOT).contains("legendaryrais"))Files.deleteIfExists(f.toPath());
            ...
            copyBundled("bedrock/LegendaryRais-Bedrock-v3.7.1-NATIVE-STABLE.mcpack",new File(packs,"LegendaryRais-Bedrock-v3.7.1-NATIVE-STABLE.mcpack"));
            copyBundled("bedrock/LegendaryRais-Geyser-v3.7.1-mappings.json",new File(maps,"LegendaryRais-Geyser-v3.7.1-mappings.json"));

            File cfg=new File(geyserDir,"config.yml");
            if(cfg.isFile()){
                YamlConfiguration y=YamlConfiguration.loadConfiguration(cfg);
                if(!y.getBoolean("enable-custom-content",false)){
                    y.set("enable-custom-content",true);
                    y.save(cfg);
                    getLogger().warning("[BEDROCK-AUTOSETUP] enable-custom-content diubah menjadi true.");
                }
            }
            getLogger().info("[BEDROCK-AUTOSETUP] v3.7.1 pack + dual mapping terpasang otomatis di "+geyserDir.getPath());
        } catch (Throwable ex) {
            getLogger().severe("[BEDROCK-AUTOSETUP] Gagal deploy otomatis: "+ex.getMessage());
        }
    }

    private void copyBundled(String resource, File target) throws IOException {
        try(InputStream in=getResource(resource)){
            if(in==null)throw new FileNotFoundException("Bundled resource missing: "+resource);
    ...
    ItemStack createSoulTideKatana(boolean menuCopy) {
        ...
        pdc.set(waterAssetVersionKey, PersistentDataType.INTEGER, 371);
    ...
    private boolean matchesLegendaryWeapon(ItemStack item, Material material, String weaponId, int modelData,
        ...
        if (itemModelKey.equals(currentModel) || oldSoulModel || modelData == readCustomModelNumber(meta)) {
            ...
            meta.getPersistentDataContainer().set(waterAssetVersionKey, PersistentDataType.INTEGER, 371);
    ...
    private ItemStack refreshWaterItem(ItemStack old) {
        ...
        if (ver != null && ver >= 371 && expected.equals(model) && old.getType() == material && readUnbreakable(meta)) return old;
    ...
    public void onWaterAssetJoin(PlayerJoinEvent event) {
        new BukkitRunnable() {
            @Override public void run() {
                ...
                if (fixed > 0) {
                    p.sendMessage("§bLegendaryRais §8• §a" + fixed + " Water Relic item diperbarui ke asset v3.7.1.");
