...
public final class LegendaryRais extends JavaPlugin implements Listener, CommandExecutor {
    ...
    public void onEnable() {
        ...
        getLogger().info("LegendaryRais v3.6.7 CROSSPLAY WATERFIX QA50 enabled. Water relic asset self-heal + rebuilt Java/Bedrock weapon rigs active.");
    ...
    private void applyV300ConfigMigration() {
        ...
        if (version >= 367) return;
        getConfig().set("config-version", 367);
    ...
    ItemStack createSoulTideKatana(boolean menuCopy) {
        ...
        applyLegendaryVisualMeta(meta, SOUL_TIDE_MODEL_DATA, "soultide", "soul_tide_sovereign_blade", false);

        PersistentDataContainer pdc = meta.getPersistentDataContainer();
        pdc.set(weaponKey, PersistentDataType.STRING, WEAPON_SOUL_TIDE);
        pdc.set(waterAssetVersionKey, PersistentDataType.INTEGER, 367);
    ...
    ItemStack createLeviathanTrident(boolean menuCopy) {
        ...
        applyLegendaryVisualMeta(meta, LEVIATHAN_MODEL_DATA, "legendaryrais", "abyss_leviathan_trident", true);
        PersistentDataContainer pdc = meta.getPersistentDataContainer();
    ...
    private boolean isLeviathan(ItemStack item) {
        return matchesLegendaryWeapon(item, Material.TRIDENT, WEAPON_LEVIATHAN, LEVIATHAN_MODEL_DATA,
                "legendaryrais:abyss_leviathan_trident", true);
    }
    ...
    private boolean isSoulTide(ItemStack item) {
        return matchesLegendaryWeapon(item, Material.NETHERITE_SWORD, WEAPON_SOUL_TIDE, SOUL_TIDE_MODEL_DATA,
                "soultide:soul_tide_sovereign_blade", false);
    }
    ...
    private boolean matchesLegendaryWeapon(ItemStack item, Material material, String weaponId, int modelData,
        ...
        boolean oldSoulModel = WEAPON_SOUL_TIDE.equals(weaponId) && "soultide:soul_tide_katana".equals(currentModel);
        if (itemModelKey.equals(currentModel) || oldSoulModel || modelData == readCustomModelNumber(meta)) {
            ...
            meta.getPersistentDataContainer().set(waterAssetVersionKey, PersistentDataType.INTEGER, 367);
    ...
    private ItemStack refreshWaterItem(ItemStack old) {
        ...
        boolean soul = WEAPON_SOUL_TIDE.equals(id) || "soultide:soul_tide_sovereign_blade".equals(model) ||
                "soultide:soul_tide_katana".equals(model) || cmd == SOUL_TIDE_MODEL_DATA;
        boolean levi = WEAPON_LEVIATHAN.equals(id) || "legendaryrais:abyss_leviathan_trident".equals(model) ||
        ...
        String expected = soul ? "soultide:soul_tide_sovereign_blade" : "legendaryrais:abyss_leviathan_trident";
        Material material = soul ? Material.NETHERITE_SWORD : Material.TRIDENT;
        if (ver != null && ver >= 367 && expected.equals(model) && old.getType() == material && readUnbreakable(meta)) return old;
    ...
    public void onWaterAssetJoin(PlayerJoinEvent event) {
        new BukkitRunnable() {
            @Override public void run() {
                ...
                if (fixed > 0) {
                    p.sendMessage("§bLegendaryRais §8• §a" + fixed + " Water Relic item diperbarui ke asset v3.6.7.");
