...
/**
 * v3.6.1 Performance Elite: 3D Java armor overlays, real MAX_HEALTH, weapon passives, unique armor skills and Bedrock parity. The two premium water relics remain in LegendaryRais core
...
public final class ArsenalExpansion implements Listener, CommandExecutor, TabCompleter {
    ...
    private static final String LEGEND_MENU_TITLE = "§0✦ Legendary Arsenal • Elemental Elite ✦";
    private final JavaPlugin plugin;
    private enum PerfMode { AUTO, LOW, BALANCED, CINEMATIC }
    private final NamespacedKey weaponKey, armorSetKey, armorPieceKey, armorHealthBaseKey, armorHealthMarkerKey, stormArrowKey, stormArrowTierKey, perfModeKey;
    ...
    public void enable() {
        ...
        clearAllArmorVisuals(); // display-overlay system stays disabled; worn armor uses native equippable model.
        runSelfCheck();
    }
    ...
    private void openLegendMenu(Player p){
        ...
        inv.setItem(4,menuItem(Material.NETHER_STAR,"§d§lLEGENDARY ARSENAL §f§lV3.5.5",List.of(
    ...
    private ItemStack createWeapon(String id){Material mat;String name,model;int cmd;List<String> lore;
        switch(id){
            case EMBERFALL->{mat=Material.NETHERITE_SWORD;name="§6§lEMBERFALL §c§lCLAYMORE §8[§6FIRE§8]";model="emberfall_claymore";cmd=EMBERFALL_CMD;lore=List.of("§7Element: §6FIRE §8• §7True 3D volcanic greatsword.","","§6I §fFlame Crescent §8• §7Right Click","§cII §fInferno Rush §8• §7Sneak + Left Click","§4III §fSolar Eruption §8• §7Sneak + Right Click","","§6Passive §8• §fHeatbreaker §7• hit ke-3 memicu combustion AOE.","§7Skill tier damage: §e70% dari Water Relic.");}
            case NOCTIS->{mat=Material.NETHERITE_SWORD;name="§5§lNOCTIS §8§lLONGSWORD §8[§5VOID§8]";model="noctis_longsword";cmd=NOCTIS_CMD;lore=List.of("§7Element: §5VOID §8• §7Elegant null-forged longsword.","","§5I §fNull Step §8• §7Right Click","§dII §fGravity Rend §8• §7Sneak + Left Click","§8III §fNull Domain §8• §7Sneak + Right Click","","§5Passive §8• §fNull Mark §7• hit ke-3 memicu void collapse.","§7Skill tier damage: §e70% dari Water Relic.");}
            case STORMPIERCER->{mat=Material.BOW;name="§f§lSTORMPIERCER §b§lARC BOW §8[§bELECTRO§8]";model="stormpiercer_recurve_bow";cmd=STORM_CMD;lore=List.of("§7Element: §bELECTRO §8• §7True 3D arc recurve bow.","","§bI §fArc Impact §8• §7Draw + shoot normally","§7Arrow meledak seperti TNT saat kena block/entity; block tidak rusak.","§fII §fChain Volley §8• §7Sneak + Left Click","§9III §fOvercharge Field §8• §7Sneak + Right Click","","§bPassive §8• §fStatic Core §7• charge ke-3 memberi electro overdrive.","§7Skill tier damage: §e70% dari Water Relic.");}
            case GAIA->{mat=Material.NETHERITE_SHOVEL;name="§f§lGAIA §a§lZEPHYR SPEAR §8[§fWIND§8]";model="gaia_war_spear";cmd=GAIA_CMD;lore=List.of("§7Element: §fWIND §8• §7Aerodynamic true 3D spear.","","§fI §fWindburst §8• §7Right Click","§aII §fSky Cutter §8• §7Sneak + Left Click","§fIII §fTempest Ascension §8• §7Sneak + Right Click","","§fPassive §8• §fTailwind §7• hit ke-4 memicu wind burst + speed.","§7Skill tier damage: §e70% dari Water Relic.");}
            case ASTRAL->{mat=Material.SHEARS;name="§b§lASTRAL §f§lFROST DAGGERS §8[§bICE§8]";model="astral_twin_daggers";cmd=ASTRAL_CMD;lore=List.of("§7Element: §bICE §8• §7Twin crystalline true 3D daggers.","","§bI §fFrost Dash §8• §7Right Click","§fII §fGlacier Fang §8• §7Sneak + Left Click","§bIII §fAbsolute Zero §8• §7Sneak + Right Click","","§bPassive §8• §fFrostbite §7• hit ke-3 freeze + shatter target.","§7Skill tier damage: §e70% dari Water Relic.");}
    ...
    private String armorSkillName(String set){return switch(set){
        case PHOENIX->"Solar Rebirth";
        case VOIDWALKER->"Event Horizon";
        case TITAN->"Colossus Bastion";
        case WATER_SOVEREIGN->"Abyssal Throne";
        default->"Aegis Constellation";
    };}
    ...
    public void stormpiercerImpact(ProjectileHitEvent e){
        ...
        double radius=tier>=2?4.25:3.65;
        double dmg=tier>=2?91.0:plugin.getConfig().getDouble("gacha.stormpiercer.arrow-impact-damage",220.0);
        // TNT-like visual/sound, intentionally no world.createExplosion so blocks are never broken.
        particle(w,"EXPLOSION_EMITTER",hit,2,.08,.08,.08,0);
        particle(w,"EXPLOSION",hit,8,.65,.45,.65,.03);
        particle(w,"ELECTRIC_SPARK",hit,64,radius*.38,.9,radius*.38,.14);
        particle(w,"FLASH",hit.clone().add(0,.4,0),2,.05,.05,.05,0);
        pulse(w,hit.clone().add(0,.35,0),"fx_thunderhead",FX_THUNDER,tier>=2?3.0f:2.5f,11,0,0);
        sound(w,hit,"minecraft:entity.generic.explode",1.35f,tier>=2?.82f:.94f);
        sound(w,hit,"minecraft:entity.lightning_bolt.thunder",.7f,1.45f);
        for(Entity ent:w.getNearbyEntities(hit,radius,radius,radius)){
            if(ent instanceof LivingEntity l&&!l.getUniqueId().equals(p.getUniqueId())){
                hurt(l,dmg,p);
                Vector kb=l.getLocation().toVector().subtract(hit.toVector());if(kb.lengthSquared()>.01)l.setVelocity(kb.normalize().multiply(.72).setY(.34));
            }
        }
        arrow.remove();
    }
    ...
    public void legendaryArmorDamageGuard(EntityDamageByEntityEvent e){
        ...
        boolean skill=raw>=55.0||internalDamage.contains(target.getUniqueId());
        double desired=switch(set){
            case TITAN->skill?8.6:4.4;
            case WATER_SOVEREIGN->skill?7.0:3.8;
            case VOIDWALKER->skill?7.6:4.2;
            case PHOENIX->skill?8.0:4.5;
            default->skill?7.2:4.0;
        };
        ...
        bar(target,"LEGEND ARMOR • "+set.toUpperCase(Locale.ROOT)+" • "+String.format(Locale.US,"%.1f",desired/2.0)+" hearts max");
    }
    ...
    public void armorDoubleSneak(PlayerToggleSneakEvent e){
        ...
        long window=plugin.getConfig().getLong("armor.input.double-sneak-window-ms",550L);
        if(now-last>window)return;
        armorPunchArmedUntil.put(p.getUniqueId(),now+plugin.getConfig().getLong("armor.input.punch-arm-ms",4500L));
        bar(p,"ARMOR ARMED • "+armorSkillName(set)+" • EMPTY-HAND PUNCH");
        sound(p.getWorld(),p.getLocation(),"minecraft:block.beacon.activate",.55f,1.55f);
    }
    ...
    public void armorPunchHit(EntityDamageByEntityEvent e){
        ...
        if(hand!=null&&hand.getType()!=Material.AIR)return;
        int cd=plugin.getConfig().getInt("armor.cooldowns."+set,switch(set){case PHOENIX->35;case VOIDWALKER->28;case TITAN->34;case WATER_SOVEREIGN->30;default->32;});
        if(!ready(p,"armor:punch:"+set,cd,armorSkillName(set)))return;
    ...
    private void armorPunch(Player p,LivingEntity target,String set){
        ...
        if(away.lengthSquared()<.01)away=p.getLocation().getDirection();away.setY(0);if(away.lengthSquared()<.01)away=new Vector(1,0,0);away.normalize();
        double kb=switch(set){case TITAN->3.15;case VOIDWALKER->2.75;case PHOENIX->2.55;case WATER_SOVEREIGN->2.65;default->2.40;};
        target.setVelocity(away.multiply(kb).setY(set.equals(TITAN)?.72:.52));
        switch(set){
            case PHOENIX->{
                target.setFireTicks(Math.max(target.getFireTicks(),100));hurt(target,10,p);
                particle(w,"FLAME",hit,48,.65,.7,.65,.08);pulse(w,hit,"fx_phoenix_rebirth",FX_PHOENIX,2.8f,9,yaw(away),0);
                sound(w,hit,"minecraft:item.firecharge.use",1.0f,.70f);
            }
            case VOIDWALKER->{
                try{target.addPotionEffect(new org.bukkit.potion.PotionEffect(org.bukkit.potion.PotionEffectType.DARKNESS,70,0,true,false,false));}catch(Throwable ignored){}
        ...
        bar(p,"ARMOR ACTIVE • "+armorSkillName(set)+" • IMPACT");
    }
