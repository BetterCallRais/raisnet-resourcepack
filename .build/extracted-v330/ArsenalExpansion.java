...
/**
 * v3.3 final realistic rebuild: fixed Bedrock/Java models, real max-health modifiers and unique armor skills. The two premium water relics remain in LegendaryRais core
...
public final class ArsenalExpansion implements Listener, CommandExecutor {
    ...
    private ItemStack createArmorPiece(String set,String piece){
        ...
        try{
            ...
            eq.setModel(new NamespacedKey("legendaryv33",set+"_set"));
    ...
    private void syncLegendaryHealth(Player p){
        try{
            ...
            // Clean the old v3.3 setMaxHealth migration if it exists.
            ...
            NamespacedKey key=new NamespacedKey(plugin,"armor_health_bonus_v33");
            for(AttributeModifier mod:new ArrayList<>(max.getModifiers())){
                if(mod.getKey().equals(key))max.removeModifier(mod);
            }

            double bonus=legendaryArmorHealthBonus(p);
            if(bonus>0.0){
                max.addTransientModifier(new AttributeModifier(key,bonus,AttributeModifier.Operation.ADD_NUMBER));
            }
    ...
    private void applyVisual(ItemMeta m,int cmd,String model){m.setUnbreakable(true);try{m.setCustomModelData(cmd);}catch(Throwable ignored){}try{m.setItemModel(new NamespacedKey("legendaryv33",model));}catch(Throwable ignored){}try{m.setEnchantmentGlintOverride(true);}catch(Throwable ignored){}}
    ...
    private ItemStack fxItem(String model,int cmd){ItemStack i=new ItemStack(Material.PAPER);ItemMeta m=i.getItemMeta();if(m==null)return i;try{m.setCustomModelData(cmd);}catch(Throwable ignored){}try{m.setItemModel(new NamespacedKey("legendaryv33",model));}catch(Throwable ignored){}i.setItemMeta(m);return i;}
