...
/**
 * v3.6.3 Resource Integrity Hotfix: 3D Java armor overlays, real MAX_HEALTH, weapon passives, unique armor skills and Bedrock parity. The two premium water relics remain in LegendaryRais core
...
public final class ArsenalExpansion implements Listener, CommandExecutor, TabCompleter {
    ...
    private final NamespacedKey weaponKey, armorSetKey, armorPieceKey, armorHealthBaseKey, armorHealthMarkerKey, stormArrowKey, stormArrowTierKey, perfModeKey, assetVersionKey;
    ...
    @Override public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(cmd.getName().equalsIgnoreCase("risrepair")){
            ...
            p.sendMessage("§aLegendaryRais repair selesai. §f"+fixed+" §aitem diperbarui ke asset v3.6.3.");
            return true;
        }
    ...
    private void openLegendMenu(Player p){
        ...
        inv.setItem(4,menuItem(Material.NETHER_STAR,"§d§lLEGENDARY ARSENAL §f§lV3.6.2",List.of(
    ...
    private ItemStack refreshLegendaryItem(ItemStack old){
        ...
        if(expansionWeapon(wid)){
            ...
            if(ver!=null&&ver>=363&&old.getType()==expected)return old;
        ...
        if(set!=null&&piece!=null&&Set.of(PHOENIX,VOIDWALKER,TITAN,CELESTIAL,WATER_SOVEREIGN).contains(set)){
            ...
            if(ver!=null&&ver>=363&&old.getType()==expected)return old;
            ItemStack fresh=createArmorPiece(set,piece);fresh.setAmount(old.getAmount());return fresh;
        }
        return old;
    }
    ...
    private String weaponId(ItemStack item){
        ...
        if(expansionWeapon(id)&&(ver==null||ver<363)){
    ...
    public void interact(PlayerInteractEvent e){
        ...
        if(right){
            ...
            String heldWeapon=weaponId(hand);
            if(set!=null&&System.currentTimeMillis()<=until&&!expansionWeapon(heldWeapon)){
                e.setCancelled(true);
    ...
    public void armorPunchHit(EntityDamageByEntityEvent e){
        ...
        if(!ready(p,"armor:punch:"+set,cd,armorPunchSkillName(set)))return;
        armorPunchArmedUntil.remove(p.getUniqueId());
        e.setCancelled(true);
    ...
    private void armorPunch(Player p,LivingEntity target,String set){
        ...
        switch(set){
            ...
            case PHOENIX->{
                ...
                target.setFireTicks(Math.max(target.getFireTicks(),120));hurt(target,14,p);
            ...
            case VOIDWALKER->{
                target.setVelocity(away.clone().multiply(.45).setY(.12));hurt(target,11,p);
                try{
            ...
            case WATER_SOVEREIGN->{
                target.setVelocity(away.clone().multiply(1.45).setY(.34));hurt(target,10,p);
            ...
            default->{
                target.setVelocity(away.clone().multiply(.85).setY(.58));hurt(target,9,p);
        ...
        switch(set){
            case PHOENIX->{
                ...
                for(Entity ent:w.getNearbyEntities(c,4,3,4))if(ent instanceof LivingEntity l&&!l.getUniqueId().equals(p.getUniqueId())){
                    l.setFireTicks(Math.max(l.getFireTicks(),100));hurt(l,8,p);
                }
            ...
            default->{
                ...
                new BukkitRunnable(){int t=0;public void run(){
                    ...
                    for(Entity ent:w.getNearbyEntities(q,4,3,4)){
                        ...
                        else if(ent instanceof LivingEntity l)hurt(l,6,p);
                    }
