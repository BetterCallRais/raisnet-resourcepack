...
OUT=Path("LegendaryRais-Java-26plus-v2.1.4-FIX.zip")
ROOT=Path(".build/generated-v214")
...
s += [cube([7.2,31,7.1],[8.8,32,8.9],"cyan"),
      cube([7.55,31.0,7.4],[8.45,32.0,8.6],"water")]
...
for side in (-1,1):
 ...
 for y in (21,25,29):
  x=cx+side*(y-19)*0.10
  a += [cube([x-0.55,y,5.7],[x+0.55,y+1.0,6.65],"bone",([x,y,6.3],"z",22.5*side))]
# extra fins
for y in (13,16,20,23):
 a += [cube([4.5,y,5.3],[6.4,y+1.1,7.0],"bone",([6.2,y+0.5,7],"z",22.5)),
       cube([9.6,y,9.0],[11.5,y+1.1,10.7],"bone",([9.8,y+0.5,9],"z",-22.5))]
...
# Validate vanilla Java model constraints before zipping.
...
for model_path in [
 ...
 for n,e in enumerate(data.get("elements",[])):
  for key in ("from","to"):
   vals=e[key]
   if any(v < -16 or v > 32 for v in vals):
    raise RuntimeError(f"{model_path.name} element {n} {key} out of bounds: {vals}")
  if "rotation" in e:
   ang=float(e["rotation"].get("angle",0))
   if ang not in allowed_angles:
    raise RuntimeError(f"{model_path.name} element {n} invalid rotation angle: {ang}")

wr(Path("pack.mcmeta"),json.dumps({"pack":{"pack_format":34,"supported_formats":{"min_inclusive":34,"max_inclusive":88},"min_format":[34,0],"max_format":[88,0],"description":"LegendaryRais v2.1.4 FIX - Java 1.21.1 to 26.2"}}))
...
Path("LegendaryRais-Java-26plus-v2.1.4-FIX.sha1").write_text(sha+"\n")
